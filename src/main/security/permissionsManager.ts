...
import {
	...
	OPEN_MACOS_SCREEN_RECORDING_PREFERENCES,
	OPEN_WINDOWS_CAMERA_PREFERENCES,
	OPEN_WINDOWS_FILESYSTEM_PREFERENCES,
...
export class PermissionsManager extends JsonFileManager<PermissionsByOrigin> {
	...
	constructor(file: string) {
		...
		ipcMain.on(OPEN_WINDOWS_FILESYSTEM_PREFERENCES, this.openWindowsFilesystemPreferences);
		ipcMain.on(OPEN_MACOS_SCREEN_RECORDING_PREFERENCES, this.openMacOSScreenRecordingPreferences);
		ipcMain.handle(GET_MEDIA_ACCESS_STATUS, this.handleGetMediaAccessStatus);
		ipcMain.handle(RESET_PERMISSION, this.handleResetPermission);
	}
	...
	private openWindowsFilesystemPreferences = () => shell.openExternal('ms-settings:privacy-broadfilesystemaccess');
	private openMacOSScreenRecordingPreferences = () => shell.openExternal('x-apple.systempreferences:com.apple.preference.security?Privacy_ScreenCapture');
	...
	private handleResetPermission = (event: IpcMainInvokeEvent, origin: string, permission: string): void => {
		if (!origin || !permission) {
			return;
		}
		const existing = this.json[origin];
...
t('main.permissionsManager.checkPermission.dialog.message.filesystem');
t('main.permissionsManager.checkPermission.dialog.detail.filesystem');
t('main.permissionsManager.checkPermission.dialog.message.display-capture');
t('main.permissionsManager.checkPermission.dialog.detail.display-capture');
...
ipcMain.on(UPDATE_PATHS, () => {
	...
	ipcMain.removeAllListeners(OPEN_WINDOWS_FILESYSTEM_PREFERENCES);
	ipcMain.removeAllListeners(OPEN_MACOS_SCREEN_RECORDING_PREFERENCES);
	ipcMain.removeHandler(GET_MEDIA_ACCESS_STATUS);
	ipcMain.removeHandler(RESET_PERMISSION);
