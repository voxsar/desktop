...
declare global {
	interface Window {
		...
		desktop: {
			...
			onAppSwitched: (listener: (appName: string) => void) => void;
			...
			openWindowsFilesystemPreferences: () => void;
			openMacOSScreenRecordingPreferences: () => void;
			getMediaAccessStatus: (mediaType: 'microphone' | 'camera' | 'screen') => Promise<'not-determined' | 'granted' | 'denied' | 'restricted' | 'unknown'>;
			resetPermission: (origin: string, permission: string) => Promise<void>;
