...
class NewServerModal extends React.PureComponent<Props, State> {
	...
	initializeOnShow = async () => {
		...
		const screenRecordingDisabled = window.process.platform === 'darwin' && await window.desktop.getMediaAccessStatus('screen') !== 'granted';
		const filesystemDisabled = false; // Filesystem access is managed at the app permission level
	...
	openWindowsFilesystemPrefs = () => {
		window.desktop.openWindowsFilesystemPreferences();
	};

	openMacOSScreenRecordingPrefs = () => {
		window.desktop.openMacOSScreenRecordingPreferences();
	};
				...
				<>
					...
					{this.props.editMode &&
						<>
							...
							<Toggle
								isChecked={this.state.permissions['display-capture']?.allowed}
								onChange={this.handleChangePermission('display-capture')}
							>
								<i className='icon icon-monitor' />
								<div>
									<FormattedMessage
										id='renderer.components.newServerModal.permissions.displayCapture'
										defaultMessage='Screen Recording &amp; Screencasting'
									/>
									{this.state.screenRecordingDisabled &&
										<small className='NewServerModal__toggle__description'>
											<FormattedMessage
												id='renderer.components.newServerModal.permissions.displayCapture.macPermissions'
												defaultMessage='Screen Recording is disabled in macOS Security settings. Click <link>here</link> to open the Screen Recording Settings.'
												values={{
													link: (msg: React.ReactNode) => (
														<a
															href='#'
															onClick={this.openMacOSScreenRecordingPrefs}
														>
															{msg}
														</a>
													),
							...
							<Toggle
								isChecked={this.state.permissions.filesystem?.allowed}
								onChange={this.handleChangePermission('filesystem')}
							>
								<i className='icon icon-folder-outline' />
								<div>
									<FormattedMessage
										id='renderer.components.newServerModal.permissions.filesystem'
										defaultMessage='File System Access'
									/>
									<small className='NewServerModal__toggle__description'>
										<FormattedMessage
											id='renderer.components.newServerModal.permissions.filesystem.description'
											defaultMessage='Required for file uploads and recordings via Uppy.'
										/>
									</small>
								</div>
							</Toggle>
