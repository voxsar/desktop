...
// Listen for window control messages from the page (bridged from executeJavaScript injections)
window.addEventListener('message', (event) => {
    if (event.source !== window) {
        return;
    }

    if (event.data?.type === 'MINIMIZE_WINDOW') {
        ipcRenderer.send(WINDOW_MINIMIZE);
    } else if (event.data?.type === 'MAXIMIZE_WINDOW') {
        ipcRenderer.send(WINDOW_MAXIMIZE);
    } else if (event.data?.type === 'CLOSE_WINDOW') {
        ipcRenderer.send(WINDOW_CLOSE);
    } else if (event.data?.type === 'SWITCH_APP') {
        ipcRenderer.send(SWITCH_APP);
    } else if (event.data?.type === 'WINDOW_DRAG') {
        ipcRenderer.send(WINDOW_DRAG, event.data.deltaX, event.data.deltaY);
    }
});
